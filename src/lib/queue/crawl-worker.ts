...
export const crawlWorker = createWorker('crawl', async (job: Job<CrawlJobData>) => {
  ...
  try {
    ...
    while (toVisit.length > 0 && processed < maxPages) {
      ...
      const jobMetadata = currentJob.metadata as Record<string, any>
      if (currentJob.status === 'failed' && jobMetadata?.errorMessage === 'Cancelled by user') {
      ...
      try {
        ...
        if (depth < maxDepth) {
          for (const link of crawled.links) {
            // Normalize URL to remove hash fragments for consistent tracking
